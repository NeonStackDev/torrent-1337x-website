...
const isTorrentPage = computed(() => route.path.startsWith('/torrent/'))
...
useHead({
  ...
  meta: computed(() => {
    ...
    if (!isTorrentPage.value) {
      tags.push({ property: 'og:url', content: `https://1337x.website${route.path}` })
    }
    return tags
  }),
  link: computed(() =>
    isTorrentPage.value
      ? []
      : [{ rel: 'canonical', href: `https://1337x.website${route.path}` }]
  ),
})
